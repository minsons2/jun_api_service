...
public class App1Controller {
	...
	@ResponseBody
	public DataResult loginByPassword(HttpServletRequest request) throws IOException {
		...
		System.out.println(JSON.toJSONString(RequestMappingExecutor.getParameters(requestWrapper)));
...
//				+ "		\"expiretime\":1596014850,\r\n"
				+ "		\"expires_in\":2592000}} ");
		return new DataResult(200,"登录成功！",json);
	}
...
//				+ "		\"expiretime\":1596014850,\r\n"
				+ "		\"expires_in\":2592000}} ");
		return new DataResult(200,"已退出登录！",json);
	}
...
//			return new DataResult(200,"首页渲染信息！",json);
//		}else {
//			return new DataResult(0,"首页渲染信息！",json);
//		}
		return new DataResult(0,"首页渲染信息！",json);
	}
	
	@RequestMapping("/api/mobile/login/sendCode")
	public DataResult sendCode(HttpServletRequest request) {
		Map param = RequestMappingExecutor.getParameters(request);
		System.out.println(JSON.toJSONString(param));
		JSONObject json= JSON.parseObject(" {} ");
		return new DataResult(200,"发送短信成功！",json);
	}
	@RequestMapping("/api/mobile/user/registerUser")
	public DataResult registerUser(HttpServletRequest request) {
		Map param = RequestMappingExecutor.getParameters(request);
		System.out.println(JSON.toJSONString(param));
		JSONObject json= JSON.parseObject(" {} ");
		return new DataResult(200,"发送短信成功！",json);
