(function (snaker) {
    ...
    designer.config = {
        ...
        path: {
            ...
            props: {
                ...
                displayName: {
                    name: "displayName", label: "显示", value: "", editor: function () {
                        return new designer.editors.textEditor()
                    }
                },
                expr: {
                    name: "expr", label: "表达式", value: "", editor: function () {
                        return new designer.editors.inputEditor()
                    }
                }
            }
    ...
    designer.util = {
        ...
        tip: function (rect, name) {
            ...
            tipDIV.style.left = (document.getElementById('snakerflow').offsetLeft + rect.attr('x')) + 'px';
            tipDIV.style.top = (document.getElementById('snakerflow').offsetTop + rect.attr('y') + 52) + 'px';
            tipDIV.style.width = 180;
            tipDIV.style.height = 120;
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    tipDIV.innerHTML = "<div style='width:180px; height:20px;border: 1px solid #d2dde2;'>" +
...
//                        "<div id='finishDIV1' style='width:180px; height:20px;'>处理结果:" + data.data.taskOperatorFlag + "</div>" +
//                        "<div id='finishDIV2' style='width:180px; height:20px;'>处理意见:" + data.data.taskOperatorMsg + "</div>" +
                        "</div>";
    ...
    designer.path = function (o, r, from, to) {
        var _this = this, _r = r, _o = snaker.extend(true, {}, designer.config.path), _path, _arrow, _text,
            _textPos = _o.textPos,
    ...
    designer.props = function (o, r) {
        var _this = this,
            _pdiv = snaker("#properties").hide().draggable({handle: "#properties_handle"}).resizable().css(designer.config.props.attr).bind("click", function () {
                return false
            }),
    ...
    designer.init = function (c, o) {
        var _w = snaker(window).width(), _h = snaker(window).height(), _r = Raphael(c, _w * 1.5, _h * 1.5),
            _states = {}, _paths = {};
        ...
        if (designer.config.editable) {
            ...
            snaker("#toolbox .node").hover(function () {
            ...
            snaker(c).droppable({
                accept: ".state",
                drop: function (c, i) {
                    snaker(_r).trigger("addrect", [i.helper.attr("type"), {
                        attr: {
