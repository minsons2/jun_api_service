	...
	class iceEditor {
		...
		menuHTML() {
			...
			this.createMenu({
				...
				name: 'fontSize',
				icon: 'fontSize',
				dropdown: '<ul class="iceEditor-fontSize"><li><span class="iceEditor-exec" data="fontSize|1">1</span></li><li><span class="iceEditor-exec" data="fontSize|2">2</span></li><li><span class="iceEditor-exec" data="fontSize|3">3</span></li><li><span class="iceEditor-exec" data="fontSize|4">4</span></li><li><span class="iceEditor-exec" data="fontSize|5">5</span></li><li><span class="iceEditor-exec" data="fontSize|6">6</span></li><li><span class="iceEditor-exec" data="fontSize|7">7</span></li><li class="iceEditor-menuTitle">文字大小</li></ul>'
			});
			...
			this.createMenu({
				...
				name: 'table',
				icon: 'table',
				dropdown: '<ul class="iceEditor-tableMain" id="' + this.tableId + '"><li><div class="iceEditor-tableBox"><div class="iceEditor-tableBgOff"></div><div class="iceEditor-tableBgOn"></div><div class="iceEditor-tableNum">表格：1×1</div></div></li></ul>',
				success: function (e, z) {
					//表格
			...
			this.createMenu({
				...
				popup: {
					...
					height: 110,
					title: '添加链接',
					content: '<div class="iceEditor-createLink"><div class="iceEditor-row"><label>创建链接：</label><input type="text" id="' + this.linkInputId + '" class="iceEditor-link" placeholder="链接地址" value=""/><a href="javascript:;" class="iceEditor-btn">确定</a></div><div class="iceEditor-row"><label><input type="checkbox" checked="checked" value="1"/> 新窗口打开</label></div></div>'
				},
				success: function (e, z) {
			...
			this.createMenu({
				...
				popup: {
					...
					height: 80,
					title: '添加音乐',
					content: '<div class="iceEditor-music"><label>音乐链接：</label><input type="text" id="' + this.musicInputId + '" class="iceEditor-link" placeholder="链接地址" value=""/><a href="javascript:;" class="iceEditor-btn">确定</a></div>'
				},
				success: function (e, z) {
				...
				success: function (e, z) {
					...
					z.id(z.filesUploadId).onchange = function () {
						...
						for (var i = 0; i < this.files.length; i++) {
							formData.append(z.filesUpload.name, this.files[i]);
						}
						formData = z.filesUpload.formData(formData);
						z.ajax({
							url: z.uploadUrl,
							data: formData,
			...
			this.createMenu({
				...
				success: function (e, z) {
					...
					z.id(z.imgUploadId).onchange = function () {
						...
						var formData = new FormData();
						for (var i = 0; i < this.files.length; i++) {
							formData.append(z.imgUpload.name, this.files[i]);
						}
						formData = z.imgUpload.formData(formData);
						z.ajax({
							url: z.uploadUrl,
							data: formData,
							success: function (res) {
			...
			this.createMenu({
				...
				icon: 'min',
				data: 'min',
				css: 'iceEditor-minWindow'
			});
			//菜单栏禁止
			this.createMenu({
				name: 'disabled',
				css: 'iceEditor-disabledMask'
			});
		}
		//格式化菜单栏
		menuFormat() {
