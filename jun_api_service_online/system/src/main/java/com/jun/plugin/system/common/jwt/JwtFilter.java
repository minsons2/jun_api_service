...
//        map.put("data/aiIndustrySystem",true);
        ...
        if(null==token){
            responseResult(httpServletResponse, Result.fail("JWTToken为空，鉴权失败"));
        ...
        if(StringUtils.isNotBlank(token)){
            try {
                if(refreshToken(token)){
                    ...
                    responseResult(httpServletResponse, Result.fail("Token非法，鉴权失败"));
                }
            } catch (BusinessException e) {
                responseResult(httpServletResponse, Result.fail(e.getMessage()));
            }
        }
    }
    ...
    public boolean refreshToken(String token) {
        ...
        try {
            if (StringUtils.isNotEmpty(cacheToken)) {
                ...
                if (JWTUtil.verify(cacheToken,"1234".getBytes())) {
